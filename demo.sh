...
# ─────────────────────────────────────────────────────────────────────
step "2 / DASHBOARD STREAMLIT — lancement immédiat"
# ─────────────────────────────────────────────────────────────────────

# Tuer tout process déjà sur 8501
...
ok "Dashboard Streamlit ouvert → http://localhost:8501  (PID=$STREAMLIT_PID)"

# ─────────────────────────────────────────────────────────────────────
step "3 / INJECTION D'ANOMALIES — clean_sales · ingest_sales"
# ─────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────
step "4 / DÉTECTION D'ANOMALIES — API REST"
# ─────────────────────────────────────────────────────────────────────
...
with urllib.request.urlopen(req, timeout=15) as r:
    d = json.load(r)
print(f"  ✔  {d['detected']} anomalies détectées → {d['saved']} sauvegardées")
...
print(f"  ✔  {len(meta)} lignes méta + {len(data)} séries exposées")
for l in lines[:10]:
    print(f"  {l}")
print("  …")
EOF
...
# ─────────────────────────────────────────────────────────────────────
step "6 / ALERTMANAGER — alertes actives + webhook"
# ─────────────────────────────────────────────────────────────────────
...
if len(alerts) > 4:
    print(f"     … et {len(alerts)-4} autre(s)")
EOF
...
with urllib.request.urlopen(req, timeout=10) as r:
    d = json.load(r)
print(f"  ✔  Webhook AlertManager → SQLite : reçu={d['received']}  persisté={d['persisted']}")
EOF
...
echo "  ╠══════════════════════════════════════════════════════════════╣"
echo "  ║  Ctrl+C  pour arrêter le dashboard Streamlit                ║"
echo "  ║  docker compose down   pour arrêter le stack Docker         ║"
echo "  ╚══════════════════════════════════════════════════════════════╝"
printf '\n'

# Garder le terminal ouvert sur les logs Streamlit
info "Logs Streamlit en direct (Ctrl+C pour quitter) :"
