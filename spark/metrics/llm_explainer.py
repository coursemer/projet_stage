...
# Délai maximum toléré pour un appel LLM (Mistral ou Ollama), quel que soit le
# comportement interne du SDK/serveur — évite que load_pipeline_data() reste
# bloqué indéfiniment si l'API est injoignable (réseau filtré, clé invalide, etc.).
_MISTRAL_TIMEOUT_SECONDS = 15
_OLLAMA_TIMEOUT_SECONDS  = 10


def _call_with_timeout(fn, timeout: float, *args, **kwargs):
    """Exécute fn dans un thread daemon et borne l'attente à `timeout` secondes.

    Un ThreadPoolExecutor utilisé en context manager attend la fin du thread
    à la sortie (shutdown(wait=True)), ce qui annule tout l'intérêt d'un
    timeout court sur un appel bloqué. Un thread daemon + Queue ne bloque ni
    l'appelant ni l'arrêt du process si l'appel ne revient jamais.
    """
    ...
    try:
        ...
        raise TimeoutError(f"Appel LLM non terminé après {timeout}s")
    if status == "error":
        raise value
    return value
...
class LLMExplainer:
    ...
    def _explain(self, anomaly: Anomaly) -> tuple[str, str]:
        ...
        if self.api_key:
            try:
                text = _call_with_timeout(self._mistral_call, _MISTRAL_TIMEOUT_SECONDS, prompt)
        ...
        try:
            text = _call_with_timeout(self._ollama_call, _OLLAMA_TIMEOUT_SECONDS, prompt)
            self._cache_set(cache_key, text)
